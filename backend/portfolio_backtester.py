...
def run_portfolio(strategy_ids, initial_bankroll=1000.0, risk_method='fixed_1'):
    ...
    for s in selected_strategies:
        ...
        if "error" not in res and "bets" in res:
            for b in res['bets']:
                ...
                # 'won' is boolean: True if the bet was a win. Use it to correctly recalculate profits.
                # We preserve the original odds so portfolio can recalculate correctly.
                all_bets.append(b)
    ...
    # Alternative bankrolls for chart comparison
    bankroll_f = initial_bankroll  # Fixed $10 per bet
    bankroll_p = initial_bankroll  # 2% proportional
    bankroll_k = initial_bankroll  # Quarter-Kelly
    ...
    for b in all_bets:
        ...
        bet_won = b.get('won', False)  # True/False from original backtest
        ...
        if risk_method == 'fixed_1':
            ...
            if bet_odds > 1.0:
                k_fraction = ((prob * bet_odds) - 1) / (bet_odds - 1)
                k_fraction = max(0.0, min(k_fraction, 0.20))
                stake = bankroll * (k_fraction / 4.0)
        
        # Safety: stake must be at least $1 and at most 10% of current bankroll
        stake = max(1.0, stake)
        stake = min(stake, bankroll * 0.10)
        
        # Only bet if we have enough bankroll
        if stake > bankroll or bankroll < 1.0:
            b['stake'] = 0.0
            b['profit'] = 0.0
            continue
        
        # CORRECT profit calculation using the actual outcome (won/lost) and odds
        if bet_won:
        ...
        # --- Alternative bankrolls (Fixed $10) ---
        stake_f = 10.0
        profit_f = stake_f * (bet_odds - 1.0) if bet_won else -stake_f
        bankroll_f += profit_f
        
        # --- Alternative bankrolls (Proportional 2%) ---
        stake_p = max(1.0, bankroll_p * 0.02)
        profit_p = stake_p * (bet_odds - 1.0) if bet_won else -stake_p
        bankroll_p += profit_p
        
        # --- Alternative bankrolls (Kelly Quarter) ---
        prob_k = float(b.get('prob', 50.0)) / 100.0
        k_frac_k = 0.0
        if bet_odds > 1.0:
            k_frac_k = ((prob_k * bet_odds) - 1) / (bet_odds - 1)
            k_frac_k = max(0.0, min(k_frac_k, 0.20))
        stake_k = max(1.0, bankroll_k * (k_frac_k / 4.0))
        profit_k = stake_k * (bet_odds - 1.0) if bet_won else -stake_k
        bankroll_k += profit_k
        ...
        # Strategy stats (use original won field for accurate win rate)
        sid = b['strategy_id']
    ...
    # Only count bets that were actually placed (stake > 0)
    placed_bets = [b for b in all_bets if b.get('stake', 0) > 0]
    wins = sum(1 for b in placed_bets if b.get('won', False))
    win_rate = (wins / len(placed_bets) * 100) if placed_bets else 0.0
    avg_odds = np.mean([b['odds'] for b in placed_bets]) if placed_bets else 0.0
    
    returns = [b['profit'] / b['stake'] for b in placed_bets if b.get('stake', 0) > 0]
    ...
    summary = {
        ...
        'profit_in_stakes': round(total_roi / 100.0, 4),
        ...
        'avg_clv': round(float(np.mean([b['clv'] for b in placed_bets if b.get('clv') is not None])), 2) if any(b.get('clv') is not None for b in placed_bets) else None,
        'bcl_percent': round(len([b for b in placed_bets if b.get('clv') is not None and b['clv'] > 0]) / len([b for b in placed_bets if b.get('clv') is not None]) * 100, 1) if any(b.get('clv') is not None for b in placed_bets) else None
    }
    
    if len(placed_bets) >= 2:
        summary['p_value'] = compute_pvalue_binomial(wins, len(placed_bets), float(avg_odds))
        try:
            brier = compute_brier_score(placed_bets)
        ...
        try:
            bootstrap = compute_bootstrap_ci(placed_bets)
        ...
        try:
            power = compute_power_analysis(summary['roi'], float(avg_odds), len(placed_bets))
        ...
        try:
            rolling = compute_rolling_roi(placed_bets)
    ...
    oos_summary = None
    if len(placed_bets) >= 20:
        n_oos = max(10, int(len(placed_bets) * 0.2))
        oos_bets = placed_bets[-n_oos:]
    ...
    ai_res = predict_strategy_sustainability(placed_bets, initial_bankroll, 1.05, ai_staking_rule, ai_stake_value, run_monte_carlo=True)
    ...
    quartiles = []
    if len(placed_bets) >= 4:
        chunk_size = len(placed_bets) // 4
        for i in range(4):
            start_idx = i * chunk_size
            end_idx = (i + 1) * chunk_size if i < 3 else len(placed_bets)
            chunk = placed_bets[start_idx:end_idx]
