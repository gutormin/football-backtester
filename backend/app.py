...
def list_leagues():
    try:
        return get_all_available_leagues()
...
def run_steam_moves_scan(req: SteamMovesRequest):
    try:
        ...
        backtester = SmartMoneyBacktester(load_league_data)
...
def predict_matchup(req: PredictRequest):
    try:
        ...
        return {
            ...
            })() if (lambda: True)(df_fix := pd.read_csv(os.path.join(DATA_DIR, "fixtures.csv"), encoding='latin1') if os.path.exists(os.path.join(DATA_DIR, "fixtures.csv")) else None) else {}
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def test_tg_arbitrage_connection():
    try:
        ...
        msg_text = format_telegram_arbitrage_tip(
            ...
            bookies_dict={'1': 'Bet365 (3.10)', 'X': 'Pinnacle (3.50)', '2': 'Betfair (3.20)'},
            profit_margin=4.20
        )
    ...
    try:
        ...
        for row in df_fixtures.to_dict('records'):
            ...
            if not pd.isna(bookie_odds) and bookie_odds > 1.0:
                ...
                # Recommended stake pct calculation
                if stakingRule == 'kelly':
                    f_star = (market_prob * bookie_odds - 1.0) / (bookie_odds - 1.0)
                    stake_pct = max(0.0, f_star) * stakeValue * 100.0
