    ...
    if (checkboxes.length > 0) {
        ...
        checkboxes.forEach((cb, index) => { 
            if (cb.checked === allChecked) {
                // Defer the click to avoid event loop swallowing
                setTimeout(() => { cb.click(); }, index * 10);
            }
        });
        
        groupEl.style.color = '#10b981';
...
# Replace the previous script block
...
with open('frontend/index.html', 'w', encoding='utf-8') as f:
    f.write(content)
print('Injected toggleGroup with setTimeout cb.click()')
